...
def new_comment(creator_id, task_id, comment):
    ...
    try:
        ...
        db.session.commit()
        sql = "INSERT INTO comments (creator_id, task_id, comment, visible) VALUES (:creator_id, :task_id, :comment, TRUE)"
...
def get_my_tasks_with_comment(user_id):
    sql = "SELECT T.task, T.priority, T.made_at, T.id, C.comment FROM tasks AS T, comments AS C WHERE T.creator_id = :user_id AND T.done = FALSE AND C.task_id = T.id AND C.visible = TRUE ORDER BY T.priority"
    return db.session.execute(sql, {"user_id":user_id}).fetchall()

def get_my_completed_tasks(user_id):
    sql = "SELECT T.task, T.priority, T.made_at, T.id, C.comment FROM tasks AS T, comments AS C WHERE T.creator_id = :user_id AND T.done = TRUE AND C.task_id = T.id AND C.visible = TRUE ORDER BY T.priority"
    return db.session.execute(sql, {"user_id":user_id}).fetchall()
